...
export class AdminTemplatesComponent {
  ...
  createNewTemplate() {
    ...
    const data = { templateName: this.newTemplateName };
  ...
  setProcessValue(id, value) {
    const values = this.selected_process.value;
    values[id] = value;
  ...
  getProcessValue(id) {
    ...
    const form = this.selected_process.form;
    for (const i in form) {
  ...
  deleteProcess(process) {
    ...
    const data = [];
    for (const index in this.selected_template.processes) {
      const item = this.selected_template.processes[index];
  ...
  moveUp(process) {
    ...
    for (let i = 0; i < this.selected_template.processes.length; i++) {
      const p = this.selected_template.processes[i];
