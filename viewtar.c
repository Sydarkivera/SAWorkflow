...
long getSizeFromExtendedHeader(char *header, int length) {
  ...
  while (offset < length) {
    ...
    if (space == -1 || equalSign == -1 || newline == -1)
      return -1;
    // test if key is "size"
    if (*(header + offset + space + 1) == 's' &&
        *(header + offset + space + 2) == 'i' &&
        *(header + offset + space + 3) == 'z' &&
        *(header + offset + space + 4) == 'e') {
      return longFromCharArray(header + offset + equalSign + 1,
                               newline - 1 - equalSign);
    }
    // goto next attribute
    offset += newline;
...
void listTarFile(FILE *fp, int indentation, char *pPath) {
  ...
  while (!feof(fp) && numberOfEmpty < 2) {
    ...
    // test if header consists of 0 only. if two zero block occure, end the file
    // reading.
    int empty = 1;
    for (int i = 0; i < 512; i++) {
      if (header[i] != 0) {
    ...
    if (!empty) {
      ...
      if (fileHeader.typeflag == '0') {
        ...
        if (dot && !strcmp(dot, ".tar")) {
          ...
          char *nextPPath =
              malloc(strlen(pPath) + strlen(fileHeader.fullName) + 1);

          strncpy(nextPPath, pPath, strlen(pPath));
          strncpy(nextPPath + strlen(pPath), fileHeader.fullName,
                  strlen(fileHeader.fullName));
        ...
        // printf("length: %s\n", fileHeader.size);
        long extendedHeaderLength =
            roundUp(octalToDecimal(fileHeader.size, 12));
        ...
        long size =
            getSizeFromExtendedHeader(extendedHeader, extendedHeaderLength);
...
void parseTarForPath(FILE *fp, char path[]) {
  ...
  while (!feof(fp) && numberOfEmpty < 2) {
    result = fread(header, 1, 512, fp);
    // test if header consists of 0 only. if two zero blocks occure, end the
    // file reading.
    int empty = 1;
    for (int i = 0; i < 512; i++) {
      if (header[i] != 0) {
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 2) {
    printf("Specify which file to inspect or remind me to write a help\n");
    return 0;
  }
